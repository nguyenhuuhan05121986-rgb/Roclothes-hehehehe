--[[	
	...
	local version 0.7.8:lerp("Realistic")
...
function RoClothes(Player)
	...
	local CVersion = '0.7.8:lerp("Realistic")'
	...
	repeat
		...
		game:GetService("TweenService"):Create(loadtxt, TweenInfo.new(1), {TextTransparency = 1}):Play()


		debounce += 1
	until debounce >= 250 or load.IsLoaded == true
	if loadtxt then
		loadtxt:Destroy()
	end
	if load.IsLoaded == false then
		...
		if oload.IsLoaded == true then
			...
			txt.Text = "Wrong version files are loaded! Please use another one!~"
			game:GetService("TweenService"):Create(txt, TweenInfo.new(1), {TextTransparency = 1}):Play()
			warn("Wrong version files are loaded! Please use another one!~")
		else
			local txt = Instance.new("TextLabel",loadui)
			txt.BackgroundTransparency = 1
			txt.TextColor3 = Color3.new(1,1,1)
			...
			game:GetService("TweenService"):Create(txt, TweenInfo.new(1), {TextTransparency = 1}):Play()
			warn("Files are not loaded correctly! Please put the RClothesContent to Content folder. Thanks!")
		end
	else
